...
def fileSplit(dayReader): #this is a csv read in file... var = csv.reader(file.csv)
	...
	for row in dayReader:
		...
		if(Record): #
			...
			#print ('' + str(timestamp) + time2 + ' ' +str(beanT) +' ' + str(mylist[3]) +' ' +mylist[5] + ' ' +mylist[20])
			outputwriter.writerow([timestamp, '' , getFloat(mylist[4]), getFloat(mylist[3]), '', getInt(mylist[5]), getInt(mylist[20]) ])#outputs the file
...
while(True):
	...
	filecount = 0
	i = 10000000
	# example line: $fileSplit(csv.reader(open('16080600.csv')))
	while((i <= 30000000)):
		#filename = (str(i), '.csv')
		try:
			#print(str(i) + '.csv')
		...
		i = i+100
		#print (i)
	print ('\nProcess Complete\n total files converted: ' + str(filecount))
