...
def fileSplit(dayReader): #this is a csv read in file... var = csv.reader(file.csv)
	...
	for row in dayReader:
		...
		if ((beanT < 385) & (beanT > 340) & (beanT < past_heat1) & (beans_cooldown == False)): #IF not picking up start of roast this line may need to be tweaked
			#  && ((past_heat1 - past_heat2) < 10?????) if@350 and if dropping and if dropping FAST
			beans_cooldown = True
			#print ('Beans are in')
			Record = False # If its dropping, a roast is done
		if((beanT < 315) & (beanT > 295) & beans_cooldown): #if a new set of beans caused the cooldown AND it's dropped to 300 #IF not picking up start of roast this line may need to be tweaked
...
while(True):
	print ('Either type \'exit\' to close this window,\nor enter the filename you wish to process, \'16042900.csv\' for example')
	...
	try:
		dayFile = open(userinput)
		dayReader = csv.reader(dayFile)
