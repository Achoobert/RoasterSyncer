...
def get_roaster_page(kb_url):
	try:
		response = requests.get(kb_url, timeout=10.10)
		if response.status_code == 200:
			print ('connected to roaster')
			return response.text
	except requests.exceptions.ConnectionError as ece:
		print('connection to roaster: error')
		pass
...
#button = Button(13, )
try:
	response = requests.get('https://www.googleapis.com/auth/drive', timeout=2.10)
	if response.status_code == 200:
		print ('Can upload to Google!')
except requests.exceptions.ConnectionError as ece:
